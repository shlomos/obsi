...
void setRealPointers(char c, int isFailure, Node *nextNode, NextStatePtr *next) {
	next->match = 0;
	next->c = c;
	next->numMessages = 0;
	if (nextNode->id < 0) {
...
void setStatesData(StateMachine *machine, ACTree *tree, PatternTable *patternTable) {
	Node *node, *next;
	Pair *pair;
	StateTable *table;
	State *state;
	...
	while (!nodequeue_isempty(&queue)) {
		...
		if (node->numGotos > 1) {
			...
			while ((pair = hashmap_iterator_next(node->gotos)) != NULL) {
				chars[i] = pair->c;
				setRealPointers(pair->c, 0, pair->ptr, &(nexts[i]));
			...
			setRealPointers(0, 1, node->failure, &nextPtr);
			...
			while (1) {
				...
				if (pair) {
					setRealPointers(lastChar, 1, next->failure, &(nexts[pathIdx]));
				...
				if (/*next == NULL || */next->hasFailInto || next->numGotos > 1) {
					// End of path
					setRealPointers(0, 0, next, &nextPtr);
					if (nextPtr.match) {
						/*
...
void generateSimpleStates(StateMachine *machine, ACTree *tree) {
	Node *node;
	Pair *pair;
	State *state;
	int id;
...
void setSimpleStatesData(StateMachine *machine, ACTree *tree) {
...
StateMachine *createStateMachine(ACTree *tree, int maxGotosLE, int maxGotosBM, int verbose) {
	...
	patternTable = patterntable_create(table->nextID);
	machine->patternTable = patternTable;
	setStatesData(machine, tree, patternTable);
...
CLICK_ENDDECLS
ELEMENT_PROVIDES(StateMachineGeneratorC)