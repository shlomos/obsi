/*
**
** $Id: mwm.c,v 1.3.8.2 2005/07/11 14:41:41 ssturges Exp $
**
...
*/
enum mwm_status mwmAddPatternEx( void *pv, unsigned char * P, int m, 
...
*/
enum mwm_status mwmAddPattern(void *pv, unsigned char *p, int m, unsigned id)
{
...
static void mwmPrepHashedPatternGroups(MWM_STRUCT * ps)
{
  ...
  */
   ...
   if( !ps->msHash ) 
   {
	   FatalError((char *)"No memory in mwmPrephashedPatternGroups()\n"
...
int mwmPrepPatterns( void * pv )
{
	...
	if(ps->msNumPatterns < 5) {
		/*victor*/ //ps->msMethod =  MTH_BM;
		/*victor*/ ps->msMethod =  MTH_MWM;
	}
	else {
		ps->msMethod =  MTH_MWM;
	}
		...
		void *data)
{
	...
	if(ps->msMethod == MTH_BM) {
				   ...
				   /* If we are case sensitive, do a final exact match test */
					if(!ps->msPatArray[i].psNoCase){
						if(memcmp(ps->msPatArray[i].psPatCase, &T[Tx-S], ps->msPatArray[i].psLen)) {
							...
							continue; /* no match, continue with this pattern, or next if n == 0 */
						}
				   }
				   nfound++;
		...
		/* Wu-Manber */
		nfound = ps->search(ps, S, n, T, match, data);
		return nfound;
	  }
}
...
{
   printf(" pattern matched: index= %d, id=%d, %s \n",   index, (int)((intptr_t)id), patArray[(intptr_t)id]);
   return 0;
}
...
BINARY  * converthexbytes( unsigned char * s)
{
   ...
   p->b   = (unsigned char*)malloc( len / 2 );
   p->blen= len / 2;
...
unsigned char* readTextFile(char* fileName)
{	
	...
	unsigned char* fileText = (unsigned char* )malloc(fileSize + 1);
	
	fread(fileText, sizeof(char), fileSize, stream);
...
int readPatternsFile(char* fileName, MWM_STRUCT* ps, int nocase)
{
	return 0;
	// FILE* stream = fopen(fileName, "rt");
	// if (stream  == NULL)
	...
	// 	int patlen = 0;
	// 	int len = strlen(line);
	// 	unsigned char* pattern = ParsePattern(line, patlen);

	// 	patArray[npats++] = (char *)pattern;

	// 	mwmAddPatternEx(ps, pattern, patlen, nocase, 0, 0, (void*)npats /* ID */, 3000 /* IID -internal id*/);
	// } 
	// while (feof(stream) == 0);
...
int CDECL main ( int argc, char ** argv )
{
   ...
   //int argc = 3;
   //char* argv[5] = {(char *)"a.ext", (char *)"-tf", (char *)"Text.txt", (char *)"-pf", (char *)"\"Patterns - 5.txt\""};

   if( argc < 3 )
   {
	  printf("usage: %s [-tf TextToSearchFile -pf PatternsFile] [-rand|-rep -n bytes -c|ch repchar -pr npats minlen ] -t|th TextToSearch -nocase [-pr numpats] -p|ph pat1 -p|ph pat2 -p|ph pat3\n",argv[0]);
	  exit(1);
   }

   /* -- Allocate a Pattern Matching Structure - and Init it. */
   ps = (MWM_STRUCT *)mwmNew();
   ...
   for(i=1;i<argc;i++)
   {
	   ...
	   {
		  i++;
		  readPatternsFile(argv[i], ps, nocase);
	   }
	   ...
	   if( strcmp(argv[i],"-ph")==0 )
	   {
		  ...
		  npats++;
		  patArray[npats] = argv[i];
		  p = converthexbytes((unsigned char *)argv[i]);
	   ...
	   if( strcmp(argv[i],"-pr")==0 )
	   {
		  ...
		  for(k=0;k<m;k++)
		  {
			 ...
			 npats++;
			 patArray[npats] = (char *)malloc( len+1 );
	   ...
	   {
		  BINARY * px = converthexbytes((unsigned char *)argv[++i]);
		  repchar = px->b[0];
		  isyn  = 1;
	   }
